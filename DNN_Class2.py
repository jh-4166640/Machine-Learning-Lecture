...
def DiffLossCEE(hat, real):
    return -1*(real/hat)
def DiffLossMSE(hat, real):
    return 2 * (hat-real)
...
class DeepNeuralNetwork():
    ...
    def ActivationFunc(self,z, var=0.01):
    ...
    def DifferentialByActiveFunc(self, arg, var=0.01):
        if self.activation == 'sigmoid':
            res = DiffSigmoid(arg)
        elif self.activation == 'ReLU':
            res = DiffReLU(arg)
    ...
    def LossDifferentialFunc(self, y_hat,y_real):
        if self.Lossfunction == 'MSE':
            return DiffLossMSE(y_hat, y_real)
        elif self.Lossfunction == 'CEE':
            return DiffLossCEE(y_hat, y_real)
    ...
    def __init__(self, filepath, hdn_node,batch=64,epoch=100,alpha=0.005,init_interval=2,init_start=-1
                 , activation='sigmoid', output_actv='sigmoid', loss='MSE'):
        """
        parameters
...
myNN = DeepNeuralNetwork(file_path, hdn_node, epoch=epoch,batch=batch,alpha=alpha
                         ,init_interval=interval,init_start=start,activation='ReLU',loss='MSE')
myNN.ModelLearning()
