...
def ActivationFunc(z, cmd, var=0.01):
    if cmd == 'sigmoid':
        p = 1 / (1+np.exp(-z))
    elif cmd == 'softmax':
        # softmax 안정화 코드 GPT 코드
        z = np.array(z)
...
epoch = 2000
alpha = 0.002
hdn_node = [20,20,16,20,40]
init_interval=2
init_start=-1
output_activeFunc = "sigmoid"
hidden_activeFunc = "ReLU"
...
train_set, validation_set, test_set = DataDivide(datas, 7, 0, 3)
...
for epc in range(0,epoch):
    ...
    for step in range(0, step_max):
        ...
        for frd_lyr in range(0,layer_size,1):
            ...
            if frd_lyr == layer_size-1:
                after_activeFn = ActivationFunc(alp, output_activeFunc)
                y_hat=after_activeFn
            else:
                after_activeFn = ActivationFunc(alp,hidden_activeFunc)
        ...
        # --- Back Propagation ---
        if output_activeFunc == "sigmoid":
            legacy = 2 * (y_hat-cur_y) * y_hat * (1-y_hat)
        elif output_activeFunc == "softmax":
    ...
    for loss_fr in range(0,layer_size,1):
        param = param_mat[loss_fr,0:hdn_node[loss_fr+1],0:hdn_node[loss_fr]+1]
        alp = param @ cx
        
        if loss_fr == layer_size-1:
            after_activeFn = ActivationFunc(alp, output_activeFunc)
            y_hat=after_activeFn
        else:
            after_activeFn = ActivationFunc(alp, hidden_activeFunc)
            after_activeFn = BiasAdd(after_activeFn, cx.shape[1])
            cx = after_activeFn
        
    loss = np.zeros_like(y_hat)
    if output_activeFunc == "sigmoid":
