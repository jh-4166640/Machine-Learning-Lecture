...
def DiffReLU(arg):
    return np.where(arg>0,1,0)
# ------------

# --- LeakyReLU ---
def LeakyReLU(x, var):
    return np.where(x>0,x,var*x)
def DiffLeakyReLU(arg, var):
    return np.where(arg>0,1,var)
# -----------------
...
class DeepNeuralNetwork():
    ...
    def ActivationFunc(self, z, cmd, var=0.1): 
        if cmd == 'sigmoid': 
            p = np.zeros_like(z, dtype=np.float64) 
            idx = z >= 0 
            p[idx] = 1 / (1 + np.exp(-z[idx]))
            idx = ~idx 
            exp_z = np.exp(z[idx])
            p[idx] = exp_z / (1 + exp_z) 
        elif cmd == 'softmax': # softmax 안정화 코드 GPT 코드 
            z = np.array(z) 
    ...
    def DifferentialByActiveFunc(self, arg, var=0.1):
    ...
    def __init__(self, filepath, hdn_node,batch=64,epoch=100,alpha=0.005,init_interval=2,init_start=-1
                 , activation='sigmoid', output_actv='sigmoid'):
        """
        parameters
        ...
        * activation : hidden layer의 활성 함수 결정
        * output_actv : output layer의 출력 활성 함수 결정
        * loss : Loss 함수 결정
        """
        ...
        self.outputActivation = output_actv
        ...
        """
        for layer_num in range(0, self.layer_size, 1):
            n_in = hdn_node[layer_num] + 1   # input 노드 수 + bias
            n_out = hdn_node[layer_num+1]    # output 노드 수
            if self.activation in ['ReLU', 'LeakyReLU']:
                rand_weight = np.random.randn(n_out, n_in) * np.sqrt(2.0 / n_in)
            elif self.activation == 'sigmoid':
                rand_weight = np.random.randn(n_out, n_in) * np.sqrt(1.0 / n_in)
            else:
                rand_weight = (np.random.rand(n_out, n_in) * init_interval) + init_start
        
            self.param_mat[layer_num, 0:n_out, 0:n_in] = rand_weight
    ...
    def MSEandACC(self, epc):
        ...
        for mse_fr in range(0,self.layer_size,1):
            ...
            if mse_fr == self.layer_size-1:
                after_activeFn = self.ActivationFunc(alp,self.outputActivation)
                y_hat=after_activeFn
            else:
                after_activeFn = self.ActivationFunc(alp,self.activation)
                after_activeFn = self.BiasAdd(after_activeFn, cx.shape[1])
                cx = after_activeFn
                
        if self.outputActivation == "sigmoid": 
            loss = np.mean((y_real - y_hat)**2) # MSE 
        elif self.outputActivation == "softmax": 
            loss = -np.mean(np.sum(y_real * np.log(y_hat)))
    ...
    def ModelLearning(self):
        ...
        for epc in range(0,self.epoch):
            ...
            for step in range(0, self.step_max):
                ...
                # --- Back Propagation ---
                if self.outputActivation == "sigmoid": 
                    legacy = 2 * (y_hat-cur_y) * y_hat * (1-y_hat) 
                elif self.outputActivation == "softmax": 
                        legacy = y_hat - cur_y # CEE
...
myNN = DeepNeuralNetwork(file_path, hdn_node, epoch=epoch,batch=batch,alpha=alpha
                         ,init_interval=interval,init_start=start,activation=act,output_actv=outact)
myNN.ModelLearning()
...
plt.title("Accuracy graphs according to epoch\n"
          +"batch size="+str(batch)+", learning rate="+str(alpha)
          +"  hidden layer="+str(hdn_node)+"\nhidden activation: "+str(act)+"  output activation: " +str(outact)
          ,fontsize=20)
