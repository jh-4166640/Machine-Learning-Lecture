...
loss_his =np.zeros((epoch, layer_size,MAX_NODE_SIZE,MAX_NODE_SIZE))
acc_his  =np.zeros((epoch, layer_size,MAX_NODE_SIZE,MAX_NODE_SIZE))
var_mat  =np.zeros((layer_size-1,MAX_NODE_SIZE-1, batch))
#layer_num = 0
...
for epc in range(0,epoch):
    ...
    for step in range(0, step_max):
        ...
        for bck_lyr in range(layer_size, 0,-1):
            ...
            param_mat[bck_lyr-1, 0:hdn_node[bck_lyr],0:hdn_node[bck_lyr-1]+1] = legacy @ var.T
